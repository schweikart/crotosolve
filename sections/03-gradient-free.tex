...
In chapter \ref{chap:background}, I have presented how quantum machine learning
uses optimizers to gradually minimize the expectation value of a parameterized
quantum circuit.
While many of these optimizers are gradient-based, the \texttt{Rotosolve}
optimizer by Ostaszewski et al. treats the different parameters of PQCs
independently.
...
I first show in section \ref{sec:gradient-free:effect} that the univariate loss
...
In section \ref{sec:constants}, I show how to determine these constants
algorithmically using a minimum number of circuit evaluations.
Finally, I outline how to minimize this reconstruction in section
\ref{sec:minimization}.
...
qubits, as shown in Figure \ref{fig:crp-circuit}.
...
$\ket 0$.
Here, $\ket{\varphi} = V \cdot CRP(\theta) \cdot U \cdot \ket{00}$ denotes the
